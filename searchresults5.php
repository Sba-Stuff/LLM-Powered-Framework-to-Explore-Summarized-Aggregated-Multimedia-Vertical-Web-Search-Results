...
// Complete version with square image thumbnails in grid layout and CSV data population
...
function createExpertTopicPages($query)
{
    ...
    foreach ($topicsToUse as $index => $topicData) {
        ...
        // Get CSV data for this page number (1-indexed)
        $pageNumber = $index + 1;
        $csvRecord = isset($csvData[$pageNumber]) ? $csvData[$pageNumber] : null;
        
        $output .= createCompactExpertPage($topic, $pageNumber, $csvRecord);
    }
...
function readCsvData($filename)
{
    ...
    if ($handle) {
        while (($line = fgets($handle)) !== false) {
            ...
            // Ensure we have at least 3 parts (title, description, keywords)
            if (count($parts) >= 3) {
                $record = [
                    'title' => trim($parts[0]),
                    'description' => trim($parts[1]),
                    'keywords' => trim($parts[2])
                ];
                $data[] = $record;
            }
        }
...
function formatKeywordsForDisplay($keywordsString)
{
    // Remove brackets and quotes, then split by comma
    $keywordsString = trim($keywordsString, '[]');
    $keywords = explode(',', $keywordsString);
    
    // Clean up each keyword (remove quotes and trim)
    $cleanedKeywords = [];
...
function createCompactExpertPage($topic, $pageNumber, $csvRecord = null)
{
    include_once("verticalsfetcher.php");
    ...
    $page .= "<h2 style='margin: 0; color: #333; font-size: 16pt;'>LLM Generated Topics</h2>";
    $page .= "</div>";
    ...
    // LLM Generated Summary with CSV data
    $page .= "<div style='border: 1px solid #ccc; padding: 10px; margin-bottom: 20px; background-color: #f9f9f9;'>";
    $page .= "<h3 style='margin-top: 0; color: #007BFF; font-size: 12pt;'>LLM Generated Summary:</h3>";
    ...
    $page .= "<td style='padding: 5px; border-bottom: 1px dashed #999; height: 25px;'>";
    
    if ($csvRecord && !empty($csvRecord['title'])) {
        $page .= "<span style='font-weight: bold;'>" . htmlspecialchars($csvRecord['title']) . "</span>";
    } else {
    ...
    if ($csvRecord && !empty($csvRecord['description'])) {
        $page .= "<span style='font-size: 10pt; line-height: 1.4;'>" . htmlspecialchars($csvRecord['description']) . "</span>";
    } else {
    ...
    if ($csvRecord && !empty($csvRecord['keywords'])) {
        $keywords = formatKeywordsForDisplay($csvRecord['keywords']);
        $keywordCount = count($keywords);
        
        // Display up to 5 keywords
        for ($i = 0; $i < 5; $i++) {
            if ($i < $keywordCount) {
                $page .= ($i + 1) . ". " . htmlspecialchars($keywords[$i]);
            } else {
                $page .= ($i + 1) . ". ________________";
            }
    ...
    // Precision Evaluation section remains unchanged
    ...
    $page .= "<p><strong>Instructions:</strong> For each result above, mark ✓ if relevant to the LLM-generated topic, ✗ if not.</p>";
    
    $page .= "<table style='width: 100%; border-collapse: collapse; margin-top: 10px;'>";
    $page .= "<tr style='background: #e9ecef;'><th>Category</th><th>Relevant (✓)</th><th>Total</th><th>Precision</th></tr>";
...
<head>
    ...
    <title>Expert Topic Creation - Compact Format with CSV Data</title>
    <style>
...
<body>
    <div class="print-controls no-print">
        ...
        <p style="margin-top: 10px; font-size: 11px; color: #666;">
            Format: 3 Web + 5 Images (square) + 5 Videos per page<br>
            CSV data loaded from lc.csv
        </p>
    </div>
    ...
    <div class="header no-print" style="text-align: center; margin-bottom: 20px; max-width: 21cm; margin-left: auto; margin-right: auto;">
        ...
        <p style="color: #666; font-size: 13px;">Each page shows 3 web, 5 image (square thumbnails), and 5 video results. CSV data is auto-populated where available.</p>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (pageCounter && pageCount > 0) {
                pageCounter.innerHTML = `Format: 3 Web + 5 Images (square) + 5 Videos per page<br>Total: ${pageCount} pages<br>CSV data loaded from lc.csv`;
            }
        });
